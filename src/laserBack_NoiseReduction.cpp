...
if (index != -1 ) // ugly check for // at beginning
{
   ...
   ros::Subscriber sub_laser = n.subscribe<sensor_msgs::LaserScan>(robotname + "/laser_back/scan", 10, laserCallback);  
   averagedScan_pub = n.advertise<sensor_msgs::LaserScan>(robotname + "/laser/scan", 1);
